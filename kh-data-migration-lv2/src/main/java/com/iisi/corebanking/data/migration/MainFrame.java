...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		setTitle("IISI CUBKH Data Migration Utility Level 2 Account Balance");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 694, 457);
		contentPane = new JPanel();
		contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
		Color mycolor = new Color(195, 195, 136).darker();
	...
	private void initBtns() {
		...
		btnConvert.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					try (FileReader frMsg = new FileReader("./saveMsgLv2.properties");) {
					...
					Properties settingTOA = new Properties();
					Properties settingCS = new Properties();
					String TOA = "TOA.properties";
					String CS = "CS.properties";
					FileReader frTOA = new FileReader(new File(chooserConfig.getSelectedFile(), TOA));
					FileReader frCS = new FileReader(new File(chooserConfig.getSelectedFile(), CS));
					settingTOA.load(frTOA);
					settingCS.load(frCS);
					...
					for (File f : fs) {
							ta.append("Read file [ " + f + " ] By insert \n");
							// 讀取設定檔
							...
							DataTransformer transformer = new DataTransformer(charsetName, setting, settingMsg, settingTOA, settingCS);
		...
		btnSaveSetting.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (FileWriter fw = new FileWriter("saveLv2.properties");) {
		...
		btnCreateSettingMsg.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Properties props = new Properties();
				props.setProperty("warning", "Currently there isn't the Error Message");
				props.setProperty("upload.company", "KH0800");
				
				try (FileWriter fw = new FileWriter("saveMsgLv2.properties");) {
					props.store(fw, new Date().toString());
	...
	private void loadSave() {
		try (FileReader fr = new FileReader("./saveLv2.properties");) {
			Properties props = new Properties();
			props.load(fr);
