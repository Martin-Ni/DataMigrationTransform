...
public class MainFrame extends JFrame {
	...
	public MainFrame() {
		...
		setTitle("IISI CUBKH Data Migration Utility Level 1");
	...
	private void initBtns() {
		...
		btnConvert.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					try (FileReader frMsg = new FileReader("./saveMsgLv1.properties");) {
						settingMsg.load(frMsg);
						frMsg.close();
						ta.append("Messages Setting Loaded \n\n");
					}catch (Exception e1) {
						System.out.println("saveMsgLv1 Load canceled");
						// load canceled
					}
		...
		btnSaveSetting.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try (FileWriter fw = new FileWriter("saveLv1.properties");) {
		...
		btnCreateSettingMsg.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Properties props = new Properties();
				props.setProperty("fields.single.error", "[WARNING11] field data length exceed ");
				props.setProperty("fields.specifid.error", "[WARNING12] (fields.expected.count)field count specified but can not be parse:");
				props.setProperty("fields.not.match.error", "[WARNING13] field count mismatch, expected");
				
				try (FileWriter fw = new FileWriter("saveMsgLv1.properties");) {
					props.store(fw, new Date().toString());
	...
	private void loadSave() {
		try (FileReader fr = new FileReader("./saveLv1.properties");) {
			Properties props = new Properties();
			props.load(fr);
		...
		ta.append("This is the Data Migratin in CUB Cambodia \n "
				+ "================= Level 1 ================= \n"
				+ "Delimiter and character\n"
				+ "1. Insert the delimiter in the multi value and sub value.\n"
				+ "2. If the field is mutli or sub value, insert the delimiter.\n"
				+ "3. The data in the field cannot over 35 character. \n"
				+ "WARNING: If UTF8 cannot run, try ISO-8859-1 \n" 
				+ "==========================V2.0.20150401============= \n" );
	}
